...
// Sheet "Gambar"  = foto produk terpusat (Produk ID, data URL). Foto yang
//     diupload lewat halaman Admin / Upload Foto masuk ke sini sehingga
//     otomatis tampil untuk semua pengunjung (tanpa deploy ulang).
//     Foto disimpan sebagai base64 di sel sheet (tanpa Google Drive), jadi
//     tidak butuh izin akses ekstra — cukup otorisasi Spreadsheets.
...
// Simpan foto ke sheet "Gambar" sebagai data URL (base64). Tanpa Google Drive,
// cukup otorisasi Spreadsheets saja. Data URL dibatasi agar muat di sel (max 50rb).
function saveImageToCell_(dataUrl) {
  const m = /^data:image\/(png|jpe?g|webp);base64,[A-Za-z0-9+/=]+$/.exec(String(dataUrl || ''));
  if (!m) return null;
  const url = String(dataUrl);
  if (url.length > 45000) return null;
  return url;
}
...
function doPost(e) {
  ...
  if (action === 'setProductImage') {
    ...
    const url = saveImageToCell_(data.dataUrl);
