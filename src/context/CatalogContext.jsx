...
import { getSheetsUrl, fetchStocks, saveProducts } from '../utils/sheets'
...
export function CatalogProvider({ children }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) setEdits(res.products || [])
      setLoaded(true)
      return res
    } finally {
  ...
  const getCategory = useCallback((slug) => baseCategories.find((c) => c.slug === slug) || null, [])
  ...
  const value = useMemo(
    ...
    [products, categories, getProduct, getProductsByCategory, getCategory, saveCatalog, refresh, syncing, loaded, isCentral],
  )
